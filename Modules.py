...
class K_attention(layers.Layer):
    ...
    def call(self, x, norm='euclidean', kernel='Gaussian', offdiag_mask=True):
        ...
        # print('*'*40)
        if kernel == 'Gaussian':
            K = tf.math.exp(-self.r_sigma * Dist**2) # need reduce mean here? scale adjustment?
        
        else:
            raiseExceptions('Kernel type not supported.')

        # K = K / tf.reduce_sum(K, axis= -1, keepdims=True)  #normalization
...
Not working in the new version of TF!
