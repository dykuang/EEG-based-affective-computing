...
#%%
import os
os.environ["CUDA_VISIBLE_DEVICES"]="0"  # place at top to take effect
...
from Models import MSBAM, EEGNet, HST_model
...
parser.add_argument('--Train_Subject', help='subject index for preTraining')
...
Train_sub = int(args.Train_Subject)
...
if args.Pretrain == 'T':
    Pretrained = True
    save_path = '/mnt/HDD/Benchmarks/DREAMER/ckpt_SI/DREAMER_S{:02d}2S{:02d}_{}_{}_{}.h5'.format(Train_sub, Test_sub, Model_Choice, Label_Code, LabelMode)
    summary_path = '/mnt/HDD/Benchmarks/DREAMER/Summary_SI/DREAMER_Pretrained_{}_{}_{}'.format(Model_Choice, Label_Code, LabelMode)

else:
    Pretrained = False
    save_path = '/mnt/HDD/Benchmarks/DREAMER/ckpt_SI/DREAMER_S{:02d}Alone_{}_{}_{}.h5'.format(Test_sub, Model_Choice, Label_Code, LabelMode)
    summary_path = '/mnt/HDD/Benchmarks/DREAMER/Summary_SI/DREAMER_Alone_{}_{}_{}'.format(Model_Choice, Label_Code, LabelMode)
...
Xtrain_raw = loadmat('/mnt/HDD/Datasets/DREAMER/S{:02d}_1min.mat'.format(Train_sub))['X'].transpose((1,0,2))
LabelTrain_raw = loadmat('/mnt/HDD/Datasets/DREAMER/Labels.mat')['Y'][Train_sub-1] - 1.0  #make it to 0 - 4

Xtest_raw = loadmat('/mnt/HDD/Datasets/DREAMER/S{:02d}_1min.mat'.format(Test_sub))['X'].transpose((1,0,2))
LabelTest_raw = loadmat('/mnt/HDD/Datasets/DREAMER/Labels.mat')['Y'][Test_sub-1] -1.0 
...
Xtrain = make_partition(Xtrain_raw, fs=Sampling_freq , seconds=Seg_len//128, stride_rate=Stride_rate/4)
...
LabelTrain= np.tile(LabelTrain_raw, (Xtrain.shape[0]//18, 1)) 
LabelTest = np.tile(LabelTest_raw, (Xtest.shape[0]//18, 1))
...
if LabelMode in ['OneHot']:
    ...
    Ytrain = LabelTrain[:,Label_choice]
    Ytest = LabelTest[:,Label_choice]
